...
* To mark the _start_ of Helium burning, we rely on the power generated by the nuclear fusion of He

> [!NOTE]
> We consider the additional condition on the power generated by He burning because H is depleted also during the Hertzsprung Gap!
...
* In this case, you might want to combine two conditions in the ```if``` statement. This is done by using the ```.and.``` operator:
```fortran
...
* For the ```power_he_burn```, you might want to consider taking the logarithm of he power and asking for it to be larger than ```1d0```. The reason for this choice is that we want the amount of power produced by the $3\alpha$ reaction to be contributing significantly to the total energy generation.
...
As you saw we are only calling GYRE every `s% x_integer_ctrl(1)` time steps. If we don't update the values each time step (because we didn't call GYRE), the values from the previous GYRE call will persist. This might be confusing as we'll have values at time steps where GYRE wasn't actually called. By setting everything to zero we ensure that we only have results for time steps where GYRE was actually called.
